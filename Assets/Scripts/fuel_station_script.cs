...
public class fuel_station_script : MonoBehaviour {
	...
	[SerializeField] float total_fuel;
	[SerializeField] Transform gauge_transform;
	[SerializeField] ParticleSystem smoke_particles;
    ...
    {
        if (collision.tag == "Player" && current_fuel >= 0)
        {
    ...
    {
        if (collision.tag == "Player" && current_fuel >= 0 && uiScr.fuel < 1)
        {
            current_fuel -= .001f;
